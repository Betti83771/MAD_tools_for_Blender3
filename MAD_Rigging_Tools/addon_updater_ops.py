...
class RTAddonUpdaterInstallPopup(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if updater.manual_only:
            ...
            atr = RTAddonUpdaterInstallPopup.bl_idname.split(".")
...
class RTAddonUpdaterCheckNow(bpy.types.Operator):
...
class RTAddonUpdaterUpdateNow(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if updater.update_ready:
            ...
            try:
                ...
                updater.print_trace()
                atr = RTAddonUpdaterInstallManually.bl_idname.split(".")
                getattr(getattr(bpy.ops, atr[0]), atr[1])('INVOKE_DEFAULT')
        elif updater.update_ready is None:
...
class RTAddonUpdaterUpdateTarget(bpy.types.Operator):
...
class RTAddonUpdaterInstallManually(bpy.types.Operator):
...
class RTAddonUpdaterUpdatedSuccessful(bpy.types.Operator):
...
class RTAddonUpdaterRestoreBackup(bpy.types.Operator):
...
class RTAddonUpdaterIgnore(bpy.types.Operator):
...
class RTAddonUpdaterEndBackground(bpy.types.Operator):
...
def updater_run_success_popup_handler(scene):
    ...
    atr = RTAddonUpdaterUpdatedSuccessful.bl_idname.split(".")
    getattr(getattr(bpy.ops, atr[0]), atr[1])('INVOKE_DEFAULT')
...
def update_settings_ui(self, context, element=None):
    ...
    if updater.error is not None:
        ...
        if "ssl" in updater.error_msg.lower():
            split.enabled = True
            split.operator(RTAddonUpdaterInstallManually.bl_idname,
                           text=updater.error)
        else:
            split.enabled = False
            split.operator(RTAddonUpdaterCheckNow.bl_idname,
                           text=updater.error)
        split = sub_col.split(align=True)
        split.scale_y = 2
        split.operator(RTAddonUpdaterCheckNow.bl_idname,
        ...
        col.scale_y = 2
        col.operator(RTAddonUpdaterCheckNow.bl_idname)
        ...
        split.enabled = False
        split.scale_y = 2
        split.operator(RTAddonUpdaterCheckNow.bl_idname, text="Checking...")
        split = sub_col.split(align=True)
        split.scale_y = 2
        split.operator(RTAddonUpdaterEndBackground.bl_idname, text="", icon="X")
        ...
        split.operator(RTAddonUpdaterUpdateNow.bl_idname, text=update_now_txt)
        split = sub_col.split(align=True)
        split.scale_y = 2
        split.operator(RTAddonUpdaterCheckNow.bl_idname,
                       text="", icon="FILE_REFRESH")
        ...
        split = sub_col.split(align=True)
        split.scale_y = 2
        split.operator(RTAddonUpdaterUpdateNow.bl_idname,
    ...
    if not updater.manual_only:
        ...
        if updater.include_branches and len(updater.include_branch_list) > 0:
            ...
            col.operator(RTAddonUpdaterUpdateTarget.bl_idname,
        ...
        col.operator(RTAddonUpdaterRestoreBackup.bl_idname, text=backup_text)
...
def update_settings_ui_condensed(self, context, element=None):
    ...
    if updater.error is not None:
        ...
        split.operator(RTAddonUpdaterUpdateNow.bl_idname, text=now_txt)
        split = sub_col.split(align=True)
        split.scale_y = 2
        split.operator(RTAddonUpdaterCheckNow.bl_idname,
                       text="", icon="FILE_REFRESH")
...
# -----------------------------------------------------------------------------
classes = (
    RTAddonUpdaterInstallPopup,
    RTAddonUpdaterCheckNow,
    RTAddonUpdaterUpdateNow,
    RTAddonUpdaterUpdateTarget,
    RTAddonUpdaterInstallManually,
    RTAddonUpdaterUpdatedSuccessful,
    RTAddonUpdaterRestoreBackup,
    RTAddonUpdaterIgnore,
    RTAddonUpdaterEndBackground
)
