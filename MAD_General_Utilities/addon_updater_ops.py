...
updater.addon = "mad_general_utilities"
...
class GUAddonUpdaterInstallPopup(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if updater.manual_only:
            ...
            atr = GUAddonUpdaterInstallPopup.bl_idname.split(".")
...
class GUAddonUpdaterCheckNow(bpy.types.Operator):
...
class GUAddonUpdaterUpdateNow(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if updater.update_ready:
            ...
            try:
                ...
                updater.print_trace()
                atr = GUAddonUpdaterInstallManually.bl_idname.split(".")
                getattr(getattr(bpy.ops, atr[0]), atr[1])('INVOKE_DEFAULT')
        elif updater.update_ready is None:
...
class GUAddonUpdaterUpdateTarget(bpy.types.Operator):
...
class GUAddonUpdaterInstallManually(bpy.types.Operator):
...
class GUAddonUpdaterUpdatedSuccessful(bpy.types.Operator):
...
class GUAddonUpdaterRestoreBackup(bpy.types.Operator):
...
class GUAddonUpdaterIgnore(bpy.types.Operator):
...
class GUAddonUpdaterEndBackground(bpy.types.Operator):
...
def updater_run_success_popup_handler(scene):
    ...
    atr = GUAddonUpdaterUpdatedSuccessful.bl_idname.split(".")
    getattr(getattr(bpy.ops, atr[0]), atr[1])('INVOKE_DEFAULT')
...
def update_settings_ui(self, context, element=None):
    ...
    if updater.error is not None:
        ...
        if "ssl" in updater.error_msg.lower():
            split.enabled = True
            split.operator(GUAddonUpdaterInstallManually.bl_idname,
                           text=updater.error)
        else:
            split.enabled = False
            split.operator(GUAddonUpdaterCheckNow.bl_idname,
                           text=updater.error)
        split = sub_col.split(align=True)
        split.scale_y = 2
        split.operator(GUAddonUpdaterCheckNow.bl_idname,
        ...
        col.scale_y = 2
        col.operator(GUAddonUpdaterCheckNow.bl_idname)
        ...
        split.enabled = False
        split.scale_y = 2
        split.operator(GUAddonUpdaterCheckNow.bl_idname, text="Checking...")
        split = sub_col.split(align=True)
        split.scale_y = 2
        split.operator(GUAddonUpdaterEndBackground.bl_idname, text="", icon="X")
        ...
        split.operator(GUAddonUpdaterUpdateNow.bl_idname, text=update_now_txt)
        split = sub_col.split(align=True)
        split.scale_y = 2
        split.operator(GUAddonUpdaterCheckNow.bl_idname,
                       text="", icon="FILE_REFRESH")
        ...
        split = sub_col.split(align=True)
        split.scale_y = 2
        split.operator(GUAddonUpdaterUpdateNow.bl_idname,
    ...
    if not updater.manual_only:
        ...
        if updater.include_branches and len(updater.include_branch_list) > 0:
            ...
            col.operator(GUAddonUpdaterUpdateTarget.bl_idname,
        ...
        col.operator(GUAddonUpdaterRestoreBackup.bl_idname, text=backup_text)
...
def update_settings_ui_condensed(self, context, element=None):
    ...
    if updater.error is not None:
        ...
        split.operator(GUAddonUpdaterUpdateNow.bl_idname, text=now_txt)
        split = sub_col.split(align=True)
        split.scale_y = 2
        split.operator(GUAddonUpdaterCheckNow.bl_idname,
                       text="", icon="FILE_REFRESH")
...
# -----------------------------------------------------------------------------
classes = (
    GUAddonUpdaterInstallPopup,
    GUAddonUpdaterCheckNow,
    GUAddonUpdaterUpdateNow,
    GUAddonUpdaterUpdateTarget,
    GUAddonUpdaterInstallManually,
    GUAddonUpdaterUpdatedSuccessful,
    GUAddonUpdaterRestoreBackup,
    GUAddonUpdaterIgnore,
    GUAddonUpdaterEndBackground
)
...
def addon_update_register(bl_info):
    ...
    updater.repo = "MAD_tools_for_Blender3"

    # updater.addon = # define at top of module, MUST be done first

    # Website for manual addon download, optional but recommended to set.
    updater.website = "https://github.com/Betti83771/MAD_tools_for_Blender3/tree/main/MAD_General_Utilities"

    # Addon subfolder path.
    # "sample/path/to/addon"
    # default is "" or None, meaning root
    updater.subfolder_path = "MAD_General_Utilities"
