...
class CopyBoneCustomPropsOperator(bpy.types.Operator):
    ...
    bl_idname = "fw.cpcp_copy"
    bl_label = "(FW) Copy Bone's custom property values"
    ...
    def execute(self, context):
        context.window_manager['fw_cpcp_clipboard'] = ""
        copy_string = "" 
        ...
        context.window_manager['fw_cpcp_clipboard'] = copy_string
...
class PasteBoneCustomPropsOperator(bpy.types.Operator):
    ...
    bl_idname = "fw.cpcp_paste"
    bl_label = "(FW) Paste Bone's custom property values"
    ...
    def execute(self, context):
        copy_string = context.window_manager['fw_cpcp_clipboard']
        for stuff in copy_string.split("__cpcp__"):
            ...
            if actual_value is not None:
                ...
                context.window_manager['fw_cpcp_clipboard'] = context.window_manager['fw_cpcp_clipboard'].replace(keyvalue[0], "").replace(keyvalue[1], "")
        return {'FINISHED'}
    
def cpcp_menu_func(self, context):
    self.layout.separator()
    copy = self.layout.operator("fw.cpcp_copy", icon='COPYDOWN')
    paste = self.layout.operator("fw.cpcp_paste", icon='PASTEDOWN')

def fw_register():
    bpy.types.WindowManager.fw_cpcp_clipboard = bpy.props.StringProperty(default="",
                                              #  name='FW CPCP clipboard', 
                                              #  desctiption="CP: Value, ...",
...
def fw_unregister():
    ...
    del bpy.types.WindowManager.fw_cpcp_clipboard

if __name__ == "__main__":
    fw_register()
    #test call
   # bpy.ops.fw.cpcp_copy()